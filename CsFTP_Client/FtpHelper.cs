...
namespace CsFTP_Client
{
    ...
    {
        private string ftpUser = String.Empty;     //用户名
        private string ftpPassword = String.Empty; //用户密码 
        private string ftpRootURL = String.Empty;  //ftp的根路径

        public FTPClient()
        {
            this.ftpUser = "hefju";// 用户名;
            this.ftpPassword = "1";// 密码;
            //    this.ftpRootURL = @"ftp://127.0.0.1/folder/";
        }
        ...
        public bool fileUpload(FileInfo localFile, string ftpPath, string ftpFileName)
        {
            ...
            catch(System.Net.WebException ex)
            {
                ...
                Trace.TraceError(ex.Message); //TraceError TraceInformation  TraceWarning
                success = false;
            }
            catch (Exception ex)
            {
